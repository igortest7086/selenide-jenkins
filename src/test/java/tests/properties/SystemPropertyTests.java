...
public class SystemPropertyTests {
    ...
    // под каждый @Tag создаем (так принято) отдельную таску в build.gradle,
    // дает дополнительные возможности для конмбинации набора тестов
    // но в простых случаях это не выглядит обязательным, достаточно при запуске из cmd указать тег напрямую
